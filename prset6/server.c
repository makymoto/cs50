...
char* indexes(const char* path)
{
    char* a = "/index.html";
    char* b = "/index.php";
    char* index_php = (char *)malloc(strlen(b) + strlen(path));
    char* index_html = (char *)malloc(strlen(a) + strlen(path));
    
    // copy the path
    strcpy(index_php, path);
    strcpy(index_html, path);
    
    // concat the index file
    strcat(index_php, b);
    strcat(index_html, a);
    
    // ensure the path exists and is readable
    if (access(index_php, F_OK) != -1 && access(index_php, R_OK) != -1)
    {
        return index_php;
    }
    else if (access(index_html, F_OK) != -1 && access(index_html, R_OK) != -1)
    {
        return index_html;
    }
    else
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = NULL;
    ...
    // create a buffer to store bytes
    BYTE buffer[BYTES];
    
    // read and count the number of bytes
    for (int c = fread(buffer, 1, BYTES, file) ; c != 0 ; c = fread(buffer, 1, BYTES, file))
    {
        // store address of the first of those bytes
...
const char* lookup(const char* path)
{
    ...
    // if there's an extension return the exact path

    // compare each extension with that of the path
...
{
    // store method and http version
    char* method = "GET";
    char* http_version = "HTTP/1.1";
    
    // check if the first 3 characters of request-line is GET
    // check also the space after GET
    if (strncmp(line, method, 3) != 0 || strstr(line, "GET ") == NULL)
    {
        error(405);
        return false;
    }
    // check if the request-line contains the HTTP/1.1
    if (strstr(line, http_version) == NULL || strstr(line, "  HTTP") != NULL)
    {
        error(505);
        return false;
    }
    
    // check if the first character of the request-target begins with /
    char* s = strchr(line, ' ');
    if (s[1] != '/')
    {
        error(501);
    ...
    // copy the request-target into the absolute path starting from the 4th character
    int i ;
    ...
    // check for the double quote in the request-target
    ...
    // search for the query in the absolute path
    if(line[i] == '?' && line[i + 1] != ' ')
    {
        abs_path[i - 4] = '?';
        abs_path[i + 1 - 4] = '\0';
        i++;
        for(int a = 0; line[i] != ' ' && line[i] != '\0'; i++, a++)
        {
            query[a] = line[i];
        }   
    }
